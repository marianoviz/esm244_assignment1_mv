...
### Introduction
In this report, I explore the variables protein, carbohydrate, fat, and energy content in three different food groups (Beef Products, Pork Products, and Poultry Products) by performing a Principal Components Analysis (PCA) and reflecting this information on a biplot. PCA is an ordination method used to visualize patterns in multivariate data that allows us to reframe this multivariate data in a simplified number of dimensions while retaining as much information as possible. This way, PCA enables us to easily explore the relationship between variables and identify multivariate patterns and outliers. 
...
#Create a subset with the variables and same observations for adding details to the biplot:
...
### Summary
Displayed PCA biplot describes 80% of variance in the analyzed data (PC1 55% and PC2 25%). The arrows convey information about the variables. The small angle separating the variables fat and energy reveals a strong positive correlation between them. In turn, these variables are negatively correlated with protein and not very highly correlated with carbohydrates (almost right angle). At the same time, the distribution of observation in the biplot enables recognizing multivariate groups and outliers. Here we can observe a big cluster of multivariate similar observations in the center of the biplot and some outlier observations extending vertically (poultry products) and horizontally (mainly beef and pork products).  
